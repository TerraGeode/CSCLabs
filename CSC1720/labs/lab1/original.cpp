/* Felecia Gonzalez
   Dr. Yong Wei
   CSC1710-03
   Lab 12:This lab will provide practice in using functions to load, process and print 1D and 2D arrays.
*/
...
int getData(string name[], int score2d[][5], string fileName) {//Read name and scores into an array (also return a list length)
   ...
   iFile >> name[i];//inputs the first name into the names array
   ...
   while (iFile && i < 28) {//runs until array full or file ends
      ...
      i++;//increments i
      iFile >> name[i];//inputs name into the names array, occurs last to prevent accidental overshoot
   }
